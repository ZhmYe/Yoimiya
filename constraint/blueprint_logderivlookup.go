...
func (b *BlueprintLookupHint) NewUpdateInstructionTree(inst Instruction, tree InstructionTree, iID int, cs *System, split bool, needAppend bool, isForwardOutput bool) {
	// depend on the table UP to the number of entries at time of instruction creation.
	nbEntries := int(inst.Calldata[1])
	//cs.initDegree(iID)
	// check if we already cached the max level
	...
	maxLevel++
	for i := 0; i < nbInputs; i++ {
		cs.AppendWire2Instruction(uint32(i+int(inst.WireOffset)), iID)
